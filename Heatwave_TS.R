...
#------------------------------------------------------------------------------------------------#
#------------------------------------------------------------------------------------------------#
mdis_dat<-read.csv("C:\\Users\\a\\Desktop\\폭염\\통계청_5세미만사망_2015_2020_revise0620.csv",fileEncoding = "euc-kr")
...
with(mdis_dat2,table(area,hwD2_maxAT_s0))
with(mdis_dat2,table(area,hwD2_maxAT_p90_s0))

d01<-subset(mdis_dat,KOR_SIDO=="서울")
d02<-subset(mdis_dat,KOR_SIDO=="부산")
d03<-subset(mdis_dat,KOR_SIDO=="대구")
d04<-subset(mdis_dat,KOR_SIDO=="인천")
d05<-subset(mdis_dat,KOR_SIDO=="광주")
d06<-subset(mdis_dat,KOR_SIDO=="대전")
d07<-subset(mdis_dat,KOR_SIDO=="울산")
d08<-subset(mdis_dat,KOR_SIDO=="세종")
d09<-subset(mdis_dat,KOR_SIDO=="경기")
d10<-subset(mdis_dat,KOR_SIDO=="강원")
d11<-subset(mdis_dat,KOR_SIDO=="충북")
d12<-subset(mdis_dat,KOR_SIDO=="충남")
d13<-subset(mdis_dat,KOR_SIDO=="전북")
d14<-subset(mdis_dat,KOR_SIDO=="전남")
d15<-subset(mdis_dat,KOR_SIDO=="경북")
d16<-subset(mdis_dat,KOR_SIDO=="경남")
d17<-subset(mdis_dat,KOR_SIDO=="제주")

#도시별 연구기간별 온도 분위수 (6~9월)
rbind(quantile(mdis_s01$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),quantile(mdis_s02$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),
      quantile(mdis_s03$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),quantile(mdis_s04$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),
      quantile(mdis_s05$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),quantile(mdis_s06$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),
      quantile(mdis_s07$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),quantile(mdis_s08$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),
      quantile(mdis_s09$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),quantile(mdis_s10$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),
      quantile(mdis_s11$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),quantile(mdis_s12$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),
      quantile(mdis_s13$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),quantile(mdis_s14$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),
      quantile(mdis_s15$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),quantile(mdis_s16$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),
      quantile(mdis_s17$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T))

rbind(quantile(d01$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),quantile(d02$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),
      quantile(d03$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),quantile(d04$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),
      quantile(d05$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),quantile(d06$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),
      quantile(d07$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),quantile(d08$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),
      quantile(d09$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),quantile(d10$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),
      quantile(d11$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),quantile(d12$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),
      quantile(d13$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),quantile(d14$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),
      quantile(d15$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),quantile(d16$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T),
      quantile(d17$maxAT,p=c(seq(0.75,0.99,0.01)),na.rm=T))
